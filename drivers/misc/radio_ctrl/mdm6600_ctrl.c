...
static void update_bp_status(void) {
	...
	printk("%s: modem status: %s -> %s [power %s]", mdmctrl,
...
static void mdm_ctrl_powerup(void)
{
	...
	printk("%s: Starting up modem.", mdmctrl);
    
	bp_status = get_bp_status();
	printk("%s: Initial Modem status %s [0x%x]",
            mdmctrl, bp_status_string(bp_status), bp_status);
    
	set_ap_status(AP_STATUS_NO_BYPASS);
	printk("%s: ap_status set to %d", mdmctrl, get_ap_status());
	msleep(100);
	set_bp_resin(0);
	msleep(100);
	...
	for (i = 0; i < loop_count; i++) {
		if (!bp_pd_ack) {
			...
			if (bp_status == BP_STATUS_SHUTDOWN_ACK) {
				printk("%s: Modem ack'd power off.", mdmctrl);
				bp_pd_ack = 1;
				set_bp_pwron(0);
			}
		}
		...
		if (gpio_value == 0) {
			printk("%s: Modem powered off.", mdmctrl);
			pd_failure = 0;
			break;
		}
...
static void __devexit mdm_ctrl_shutdown(struct platform_device *pdev)
{
	...
	printk("%s: Shutting down modem.", mdmctrl);
	...
	if (!pd_failure) {
		...
		if (pd_failure) {
			printk("%s: Resetting unresponsive modem.", mdmctrl);
			set_bp_resin(1);
			pd_failure = bp_shutdown_wait(5);
		}
	}
...
static void mdm_ctrl_dump_log(void)
{
	printk("%s: Dumping modem log", mdmctrl);
